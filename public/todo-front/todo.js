...
const weahterInit = () => {
    ...
    if (!localStorage.getItem('coords')) {
        ...
        .then(res => res.json())
        .then(json => Weather.innerHTML = `${json.main.temp} @ ${json.name}`).catch(err => console.log(err));
            console.log(localStorage.getItem('coords'))}, (err) => console.log(err))}
    else {
...
const signUpUser = (e) => {
    e.preventDefault();
    let userName = Array.from(e.target.elements).find(v => v.name == 'user').value;
    let passWord = Array.from(e.target.elements).find(v => v.name == 'pass').value;
    let passConfirm = Array.from(e.target.elements).find(v => v.name == 'pass-confirm').value;
    signUpForm.style.display = 'none';
    loadingSignUp.style.display = 'block';
   fetch('https://node-todo-kmuse.herokuapp.com/signup', {
       method: 'POST',
       body:JSON.stringify({ name:userName, password:passWord, passconfirm:passConfirm}),
   ...
   .then(resultJson => {
    ...
    if (resultJson.error) {
        ...
        Array.from(e.target.elements).forEach(v => { if (v.tagName == 'INPUT') v.value = ''; });
...
const submitUser = (e) => {
    e.preventDefault();
    let userName = Array.from(e.target.elements).find( v => v.name == 'name').value;
    let passWord = Array.from(e.target.elements).find( v => v.name == 'pass').value;
    askNameForm.style.display = 'none';
...
const submitToDo = (e) => {
    e.preventDefault();
    let todoValue = Array.from(e.target.elements).find( v => v.name == 'todo').value.trim();
...
const toggleCheck = (e) => {
    ...
    fetch(`https://node-todo-kmuse.herokuapp.com/user/${targetUserId}/todo/${targetId}`, {
        method:'PUT',
...
const LogOut = () => {
     ...
     .then( _ => {alert('Good-Bye') ; checkUser()})
     .catch(err => console.log(err));
}
...
activeShowing.addEventListener('click', (e) => {
    Array.from(document.querySelectorAll('li')).forEach(v => {
        if(Array.from(v.childNodes).find(x => x.tagName == 'SPAN').className != 'completed') {
          if( v.classList.contains('hide') ) { v.classList.remove('hide') };
        };
        if( Array.from(v.childNodes).find(x => x.tagName == 'SPAN').className == 'completed') {
          if(!v.classList.contains('hide')) { v.classList.add('hide') };
        };
     });
});
